...
exports.inject = function (dispatchFunc, options) {
    ...
    return new Promise((resolve) => {
        ...
        const socket = Response.nullSocket();
        const req = new Request(options, socket);
        const res = new Response(req, resolve);
        res.assignSocket(socket);

        // ServerResponse (res) handles socket close on its own, but req needs a little extra help, typically provided by HttpServer.
        socket.on('close', () => req.destroy());
